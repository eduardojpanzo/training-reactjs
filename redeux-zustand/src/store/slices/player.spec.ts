...
describe("player slice", () => {
  it("should be able to aply", () => {
    const state = reducer(exampleSate, play([1, 2]));
  ...
  it("should be able to next video automatically", () => {
    const state = reducer(exampleSate, next());
  ...
  it("should be able to jumb to next module automatically", () => {
    const state = reducer({ ...exampleSate, currentLessonIndex: 1 }, next());
  ...
  it("should not update the current module and lesson index if there is no next lesson avaliable", () => {
    const state = reducer(
      { ...exampleSate, currentLessonIndex: 1, currentModuleIndex: 1 },
      next()
    );
